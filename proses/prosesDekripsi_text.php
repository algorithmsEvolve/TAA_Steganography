...
function error_found(){
  header('Location: ../view/decrypt_v.php?x=2');
  die;
}
...
for ($x = 0; $x < ($width*$height); $x++) { //Loop through pixel by pixel
  ...
  if ($count == 8) { //Every time we hit 8 new digits, check the value
      if (toString(substr($real_message, -8)) === '|') { //Whats the value of the last 8 digits?
          ...
          session_start();
          $_SESSION['real_messageY'] = $real_messageY;

          /*decrypt with AES258*/
          $c = '12345678912345678912345678912345';
          $hasil3 = $real_messageX['1'];
        // echo bin2hex($hasil);
          $aes = new Aes($c);
          ...
          if ($real_messageX['1']==$password){
            header('Location: ../view/decrypt_v.php?x=1');
            die;
          }
          else {
            header('Location: ../view/decrypt_v.php?x=2');
